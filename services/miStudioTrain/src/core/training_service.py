# core/training_service.py - Memory Optimized Version
...
class MiStudioTrainService:
    """Enhanced training service with dynamic model loading and memory optimization"""
    ...
    def __init__(self, base_data_path: str = "/data"):
        ...
        logger.info("🏋️ miStudioTrain API Service v1.2.0 initialized (Memory Optimized)")
    ...
    async def run_training_job(self, job_id: str):
        """Execute the training job with memory optimization"""
        job_info = self.active_jobs[job_id]
        ...
        try:
            ...
            if device.type == "cuda":
                ...
                logger.info(f"GPU memory before activation extraction: {mem_info['allocated_gb']:.1f}GB allocated")
            ...
            if device.type == "cuda":
                torch.cuda.empty_cache()
                mem_info = GPUManager.get_memory_info(gpu_id)
                logger.info(f"GPU memory after activation extraction: {mem_info['allocated_gb']:.1f}GB allocated")
            ...
            for epoch in range(config.max_epochs):
                ...
                if device.type == "cuda" and epoch % 5 == 0:
                    mem_info = GPUManager.get_memory_info(gpu_id)
                    logger.info(f"Epoch {epoch}: GPU memory utilization: {mem_info['utilization_pct']:.1f}%")
            ...
            if device.type == "cuda":
                torch.cuda.empty_cache()
                final_mem = GPUManager.get_memory_info(gpu_id)
                logger.info(f"Final GPU memory usage: {final_mem['allocated_gb']:.1f}GB")
