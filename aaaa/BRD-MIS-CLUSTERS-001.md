...
## Addendum — 2026-08-19: two clustering gaps closed

Found while mapping which documents carry a clustering requirement. Both were
places where the chain's own discipline was stated but not enforced.

### 1. The causal-language audit did not reach cluster surfaces

`test_causal_language_audit.py` globbed the circuit, J-space and `millm_*`
corpora and reached **none** of the twelve cluster modules — including both REST
endpoints and the `groups` MCP tool, which an agent reads and relays verbatim.
The clean separation in these documents (causal, rung and intervention appear
zero times across all thirteen) held by authorship alone.

Now discovered under **both** naming conventions. The Feature Groups → Clusters
rename was UI-only (IDL-28), so six of the twelve are still `feature_group*`; a
`cluster_*` glob alone reads as thorough and misses the service that does the
grouping. Verified by planting an overclaim in each convention.

### 2. Cluster-level circuit edges never reached the hazard analysis

BR-016 makes a cluster a first-class circuit member, so a circuit's edges can be
cluster-level. `detect_hazards` keys on `(layer, feature_idx, …)`, a cluster
endpoint has no `feature_idx`, and every such edge was skipped.

The failure was silent and backwards: an edge at rung >= 2 carries a MEASURED
effect size, where the fallback is a weight-prior heuristic labelled
`heuristic`. Steering a cluster-membered circuit therefore discarded precisely
the strongest evidence available and reported an empty hazard list — which reads
as "safe", not as "not analysed".

`expand_cluster_edges` now resolves a cluster edge into the feature pairs it
stands for, each inheriting the edge's rung and effect size, bounded to the
members actually being steered. Edges whose profile is missing or empty are
