...
class TestTheFitWorkerUsesTheSharedHelper:
    ...
    def test_the_fit_worker_CALLS_the_shared_helper(self):
        ...
        commit site and this fails.
        """
        import dis

        from src.workers.jlens_fit_tasks import _fit_and_publish

        # THE NAME BEING PRESENT IS NOT ENOUGH — and the first version of this
        # test made exactly that mistake. `co_names` contains the name because
        # the function IMPORTS it, so replacing `defer_consumer_checks(report)`
        # with a bare `report` at the commit site left this green. A guard that
        # an unused import satisfies is the fail-open shape all over again.
        #
        # An import BINDS the name (STORE_FAST); only a use LOADS it. So a LOAD
        # of this name is the evidence that it is actually invoked.
        loads = [
            i
            for i in dis.get_instructions(_fit_and_publish)
            if i.opname.startswith("LOAD") and i.argval == "defer_consumer_checks"
        ]
        assert loads, (
            "the fit worker imports defer_consumer_checks but never uses it; "
        ...
        # above is checking the wrong body.
        assert any(
            i.argval == "commit" for i in dis.get_instructions(_fit_and_publish)
