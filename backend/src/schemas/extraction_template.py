...
class ExtractionTemplateCreate(ExtractionTemplateBase):
    ...
    extra_metadata: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional metadata (filter settings, NLP config, etc.)")
...
class ExtractionTemplateResponse(ExtractionTemplateBase):
    ...
    micro_batch_size: Optional[int] = Field(None, description="GPU micro-batch size for memory efficiency")
    top_k_examples: int = Field(..., description="Number of top activating examples to save")
    context_prefix_tokens: int = Field(25, description="Tokens before prime token")
    context_suffix_tokens: int = Field(25, description="Tokens after prime token")
