        ...
        try:
            ...
            async with AsyncSessionLocal() as db:
                        ...
                        try:
                            ...
                            if loop.is_running():
                                _asyncio.ensure_future(
                                    _update_db("pushing", pct_done, progress.features_pushed)
                                )
                        except Exception:
                            pass
                ...
                if result.success:
                    ...
                    await _update_db("completed", 100, result.neurons_created)
                    ...
                    await _update_db("failed", 0, error=result.error_message)
                    emit_neuronpedia_push_progress(
                        push_job_id=push_job_id,
                        sae_id=sae_id,
                        stage="failed",
            ...
            await _update_db("failed", 0, error=str(e))
            emit_neuronpedia_push_progress(
                push_job_id=push_job_id,
                sae_id=sae_id,
                stage="failed",
