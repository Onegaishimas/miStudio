  ...
  if (isLoadingTokenAnalysis) {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-3">
        ...
        <div className="text-slate-600 dark:text-slate-400 text-sm">Analyzing tokens...</div>
      </div>
    );
  }
  ...
  if (tokens.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 space-y-2">
        ...
        <div className="text-slate-600 dark:text-slate-400">No meaningful tokens found</div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-4">
        ...
        {showFilters && (
          <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3">
            <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer hover:text-slate-900 dark:hover:text-white">
              <input
                type="checkbox"
                checked={filterSpecial}
                onChange={(e) => setFilterSpecial(e.target.checked)}
                className="rounded border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-white dark:focus:ring-offset-slate-900"
              />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-4">
          ...
          <div className="text-2xl font-semibold text-slate-800 dark:text-slate-200">{summary.total_examples}</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-4">
        ...
        <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-4">
          ...
          <div className="text-2xl font-semibold text-slate-800 dark:text-slate-200">{summary.filtered_token_occurrences}</div>
        </div>
        <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-4">
        ...
        <div className="overflow-x-auto" style={{ maxHeight: '600px' }}>
          <table className="w-full">
            <thead className="bg-slate-100 dark:bg-slate-800/50 sticky top-0 z-10">
              <tr>
                <th className="text-right px-4 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 w-16">
                  Rank
                </th>
                <th className="text-left px-4 py-3 text-xs font-medium text-slate-600 dark:text-slate-400">
                  Token
                </th>
                <th className="text-right px-4 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 w-24">
                  Count
                </th>
                <th className="text-right px-4 py-3 text-xs font-medium text-slate-600 dark:text-slate-400 w-32">
                  Percentage
                </th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end space-x-3">
                        {/* Bar visualization */}
                        <div className="flex-1 max-w-[80px] bg-slate-200 dark:bg-slate-700/50 rounded-full h-2">
                          <div
