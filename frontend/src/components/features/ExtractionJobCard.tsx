...
import { Zap, Loader, CheckCircle, XCircle, Trash2, Clock, ChevronDown, Search, ArrowUpDown, Star, ArrowUp, ArrowDown, RefreshCw, List, Layers, Activity, Copy, Check, Brain, StopCircle, Play, RotateCcw } from 'lucide-react';
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
            ...
            <>
              ...
              {extraction.nlp_status === 'completed' && (
                <button
                  ...
                  disabled={isNlpResetting}
                  aria-label="Re-run NLP analysis from scratch"
                  className={`group flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium
                    ...
                    isNlpResetting
                      ...
                      : 'border-cyan-500/25 bg-cyan-500/5 text-cyan-700 dark:text-cyan-400 '
                        + 'hover:border-cyan-500/50 hover:bg-cyan-500/15 hover:shadow-sm hover:shadow-cyan-500/20'
                  }`}
                  title="Discards the existing NLP analysis for this extraction and computes it again from scratch"
                >
                  {isNlpResetting ? (
                    <Loader className="w-3.5 h-3.5 animate-spin" />
                  ) : (
                    // Winds backwards on hover — the gesture says "run it again".
                    <RotateCcw className="w-3.5 h-3.5 transition-transform duration-500 group-hover:-rotate-180" />
                  )}
                  <span>{isNlpResetting ? 'Re-running\u2026' : 'Re-run NLP'}</span>
                </button>
              )}
            </>
          )}
          ...
          {isCompleted && (
            <button
              ...
              aria-label={isExpanded ? 'Hide the feature browser' : 'Browse the features from this extraction'}
              className="group flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium
                border border-emerald-500/25 bg-emerald-500/5 text-emerald-700 dark:text-emerald-400
                hover:border-emerald-500/50 hover:bg-emerald-500/15 hover:shadow-sm hover:shadow-emerald-500/20
                transition-all duration-200"
              title={isExpanded
                ? 'Hide the feature browser'
                : 'Open the feature browser for this extraction — search, filter and inspect individual features'}
            >
              {/* One icon that ROTATES rather than two that swap: the turn itself
                  shows the panel opening, where a swapped glyph just blinks. */}
              <ChevronDown
              ...
              <span>
                {isExpanded
                  ? 'Hide features'
                  : extraction.statistics?.total_features
                    ? `Browse ${extraction.statistics.total_features.toLocaleString()} features`
                    : 'Browse features'}
              </span>
            </button>
          )}
