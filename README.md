...
**5. Steering** — The definitive proof of a research claim. Select features, configure strengths (positive to amplify, negative to suppress), write a prompt, and generate. miStudio's steering system is built around a grid testing approach rather than a single slider: test up to four features simultaneously at multiple strength values in one generation pass, compare features side-by-side at the same strength, or combine multiple features to discover synergistic effects. Steering runs asynchronously against a dedicated GPU partition so the interactive steering UI stays responsive while generation is happening.
