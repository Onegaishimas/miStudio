...
## Current Status
- **Phase:** Post-increment sensing enhancements (2026-07-17) — 4 goal items shipped; review rounds in progress (R1 done: 23 findings/17 fixed)
...
- **New BRD (2026-07-18):** BRD-MIS-CIRCUITS-001 — multi-SAE cross-layer steering + automatic circuit discovery (`0xcc/prds/BRD-MIS-CIRCUITS-001.md`). 14 BRs / 8 RSKs. Locked: validation badge-not-gate; seeded+open discovery; "Circuits" vocabulary; miStudio-only (miLLM multi-SAE runtime → follow-on BRD-MILLM-CIRCUITS-001; BR-014 per-layer v1 projection bridges meanwhile). Key facts: hooks already register per-layer but share ONE SAE decoder; extraction discards per-token codes (capture pass needed); existing ablation endpoint is a heuristic FAKE (BR-009 remediates). Next: 002_create-project-prd.md.
