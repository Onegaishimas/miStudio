...
- Because `kind` is a namespaced string (`mistudio.cluster-definition`), other tools (miLLM import is already planned) can consume the format without naming collision — the vendor-prefixed `kind` + published JSON Schema (`$id: https://raw.githubusercontent.com/hitsainet/miStudio/main/docs/schemas/cluster-definition-v1.json`) is the interop contract; the Hub is just transport.
