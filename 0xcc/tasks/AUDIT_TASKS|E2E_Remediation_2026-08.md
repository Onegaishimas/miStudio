...
- [x] 14.4 ✅ **Third sweep — five quiet-failure defects.** **-087**: `_find_matching_attr` looped `dir()`, which is alphabetical, discarding the preference order `LAYER_NORM_PATTERNS` documents — it returned `input_layernorm` (pre-attention) where the constant asks for `post_attention_layernorm`, the residual-stream point. Its own docstring asserted order did not matter, and `_is_transformer_layer` *did* iterate in order, so the two disagreed about the same model. **-104**: `delay()` fired before the Celery id was written, leaving a window where a GPU training ran with nothing recording its id — so no janitor could revoke it. Now `apply_async(task_id=...)` with a pre-generated id, persisted first. **-045**: both seed migrations matched on `name` alone with a `pass` downgrade, so a user template sharing a name lost its edits permanently; guarded on `is_system`. **-039**: a downgrade deleted every tokenization for any dataset with `tokenized_path` set, under a docstring promising it only removed its own rows — it has no way to know that. It now refuses. **-096**: two `update_state` calls passed a bare meta dict, which REPLACES the previous one and erased the liveness stamp the janitor reads; and the stuck-job message interpolated `job.status` after setting it to FAILED, so every diagnostic read "stuck in FAILED". — MIS-E2E-087, -104, -045, -039, -096
...
| `backend/src/ml/layer_discovery.py` | `_find_matching_attr` iterates the pattern list in preference order instead of `dir()` alphabetically (-087) |
| `backend/src/api/v1/endpoints/trainings.py` | Celery id generated and persisted before dispatch (-104) |
| `backend/alembic/versions/n1o2p3q4r5s6_*.py`, `o2p3q4r5s6t7_*.py` | Seed UPDATEs guarded on `is_system` (-045) |
| `backend/alembic/versions/2e1feb9cc451_*.py` | Downgrade refuses instead of deleting rows it cannot prove it created (-039) |
| `backend/src/workers/jlens_fit_tasks.py` | Both bare `update_state` calls go through `beat()` so the liveness stamp survives (-096) |
| `backend/src/workers/cleanup_stuck_enhanced_labeling.py` | Captures the stuck status before overwriting it with FAILED (-096) |
| `backend/tests/unit/test_wave7_group3.py` | **New.** 12 tests over all five, incl. a fixture whose names sort the wrong way on purpose |
| `backend/src/ml/sparse_autoencoder.py` | Reconstruction, zero-ablation and the ghost gradient all measured in the space the L1 term lives in; `SkipAutoencoder.loss_zero` made comparable across architectures (-086) |
| `backend/tests/unit/test_sparse_autoencoder.py` | The defect-pinning `test_reconstruction_loss_is_mse` replaced; new coverage for the ghost gradient, the zero-ablation space, and `SkipAutoencoder` (which had none — C192 survived on that) |
| `backend/src/mcp_server/health_gate.py` | Catches `httpx.InvalidURL` (not an `HTTPError` subclass) plus a catch-all, so the gate always returns a reason instead of raising (-116) |
| `backend/src/mcp_server/config.py` | `millm_api_url_override` — a per-instance override, so nothing needs to write to `os.environ` (-115) |
| `backend/src/mcp_server/tools/howto.py` | No longer mutates the process environment (-115) |
| `backend/tests/unit/test_mcp_gate_and_purity.py` | **New.** 9 tests: the gate answers for malformed/unroutable/unconfigured URLs, and listing the tools leaves `MILLM_API_URL` untouched |
...
| `backend/src/schemas/cluster_profile.py` | `MemberMeta` extras capped at 8 KB; the declared fields were bounded and the undeclared ones were not (-042) |
| `frontend/tsconfig.test.json` | Rewritten — overrides the inherited `exclude` that made it check nothing, and drops the uninstalled `@types/node` that made it fail to load (-021) |
...
| C183 | Layer discovery back to `dir()` order | ✅ 2 failures |
| C184 | Dispatch before persisting the Celery id | ✅ 2 failures |
| C185 | The `is_system` guard removed from a seed migration | ✅ 1 failure |
| C186 | The tokenization downgrade deletes again | ✅ 1 failure |
| C187 | A bare `update_state` drops the heartbeat | ✅ 1 failure |
| C188 | The stuck-job message reads the status after overwriting it | ✅ 1 failure |

| C189 | Reconstruction back to raw space | ✅ 1 failure |
| C190 | Ghost gradient encodes raw `x` again | ✅ 2 failures |
| C191 | Zero-ablation back to raw space | ⚠️ **survived** — nothing tested `loss_zero`'s space. Test added; re-run ✅ 2 failures |
| C192 | `SkipAutoencoder` reconstruction back to raw | ⚠️ **survived** — the subclass had no loss tests at all, the test-side twin of "fixed one representative". Tests added; re-run ✅ 1 failure |
| C193 | `SkipAutoencoder` baseline back to `skip_scale * x` | ⚠️ **survived twice** — and chasing why found a real defect: at the default `skip_scale=1.0` that baseline is `mse(x, x)` = **0.0** for every Skip-SAE training, in a column shared with other architectures. Fixed; re-run ✅ 2 failures |

| C194 | Remove the `InvalidURL` clause from the gate | ⚠️ **survived twice.** First because the catch-all answered identically — the test only asserted "some reason", so the clause was not load-bearing. Then because my chosen URL (`http://[not a url`) does not actually raise `InvalidURL` on httpx 0.28.1; it is accepted and fails later as a `ConnectError` the old handler already caught. An invalid **port** (`http://host:notaport`) is what reaches the clause. Re-run ✅ 1 failure |
| C195 | `howto` mutates `os.environ` again | ✅ 2 failures |
| C196 | The settings override is ignored | ✅ 2 failures |
| C197 | Remove both the `InvalidURL` clause and the catch-all | ✅ 1 failure |
