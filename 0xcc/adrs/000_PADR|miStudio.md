...
### IDL-27: Steering UX — frequency-based auto-baseline, blended/compare, 20 features
...
- The Feature Groups → Steering hand-off widens its selection map to preserve `activation_frequency` + `max_activation` (already returned by the members endpoint); the SAE feature browser gains `activation_frequency` on `SAEFeatureSummary`.
- The constants (2.9/2.6) are treated as global this phase; documented as SAE-local, with per-SAE calibration a future option.

**Tradeoffs:**
- Frequency-only is honest to the evidence but under-fits SAEs with non-unit-norm decoders; the manual strength control and fallback always remain, so a wrong baseline is never blocking.
- A 20-entry palette means colors can repeat past the distinct-hue budget; acceptable since colors are cosmetic once uniqueness is dropped.
- Blended-vs-Compare reuses existing endpoints (zero new backend surface) at the cost of not offering a genuinely sequential mode some users may expect.
...
**Decision:**
1. **One unified steering core + three per-type resolvers.** The circuit, cluster, and feature steering paths are the same additive residual-stream math (`Σ strength·W_dec[:,idx]`), differing only in member resolution. A shared `steering_core.build_steer_generator` + `resolve_circuit/cluster/feature_members` drives all three; calibration's `_build_generation_fns` is refactored to use it (judge gate before model load, 5-tuple preserved, generation moved not rewritten). Two conventions: the hook target is the **WHOLE decoder-layer output (resid_post)** — the same point the live feature/cluster serve path steers at — and the RECORDER owns the dial multiply. **Hardware E2E correction:** the original design standardized on the discovered "residual" module, but on LFM2 that resolves to a post-attention RMSNorm, and a vector added at a normalization layer is renormalized away — the recorder produced steered output byte-identical to the baseline at every dial. The whole-layer output is the correct, serve-matching target; the earlier "residual" convention was never validated because calibration's own hardware run stopped at `judge_unreliable` before measuring a real steering effect.
